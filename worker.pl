...
#         This is my worker.pl for reference
...
sub processRequest {
	my ($state, $tag, $timeout, $flags, $msgref) = @_;
	...
	if ($timeout) {
		...
		my $cmdref = shift(@$request);
		wlog DEBUG, "Cmd is $$cmdref\n";
		if (exists($HANDLERS{$$cmdref})) {
...
sub process {
	...
	if ($reply) {
		if (exists($REPLIES{$tag})) {
			...
			wlog(WARN, "received reply to unregistered command (tag=$tag, msg=$$msgref). Discarding.\n");
			return;
		}
	}
	else {
	...
	$$cont{"dataIn"}($cont, $tag, 0, $flg, $msgref);
...
sub registerCBDataIn {
	my ($state, $tag, $timeout, $flags, $replyref) = @_;
...
sub heartbeatCBDataIn {
	...
	if ($timeout) {
		...
		dieNicely("Heartbeat failed: $$replyref");
	}
	else {
...
sub globuscopy {
  my ($jobid, $src, $dst) = @_;
  my $GLOBUS_USER = "ez5504";#qw(GLOBUS_USERNAME);
  if($GLOBUS_USER eq "") {
    wlog DEBUG, "No Globus username found\n";
    dieNicely("GLOBUS_USERNAME is undefined.");
  }
  my $LOCAL_ENDPOINT = "home";#qw(GLOBUS_ENDPOINT);
  if ($LOCAL_ENDPOINT eq "") {
    wlog DEBUG, "No local Globus endpoint found in path\n";
    dieNicely("No local globus endpoint found in path");
  }
  my $ssh = "ssh -i /home/jake/.ssh/globus $GLOBUS_USER";
  my $cmd = join('',$ssh,'@cli.globusonline.org scp ',
                    "$src $GLOBUS_USER","#$LOCAL_ENDPOINT",":$dst");
  wlog DEBUG, "system call : $cmd \n";
  my $sysrc = system($cmd);
  wlog INFO, "File successfully staged in with Globus : $sysrc \n";
}
...
#    #mkfdir($jobdir, $dst);
#    system("wget $src -P $dst");
#    wlog DEBUG, "File retrieved with wget...\n";
#    my $handle;
#		if (!open($handle, ">", "$dst")) {
#			dieNicely("Failed to open $dst: $!");
#  }
...
sub stagein {
    ...
    elsif ($protocol eq "go") {
      my $dst = $$STAGED[$STAGEINDEX];
      my $src = $$STAGE[$STAGEINDEX];
      mkfdir($jobid, $dst);
      my ($goprotocol, $endpoint, $gopath) = urisplit($src);
      $endpoint .= "/";
      $endpoint .= $gopath;
      wlog INFO, "$jobid src : $endpoint dst : $dst\n";
      if(!globuscopy($jobid, $endpoint, $dst)) {
        wlog DEBUG, "$jobid : Error in staging in $path -> $dst with Globus\n";
        sub {
          my ($err, $msg) = @_;
          if(!$err) {
            stagein($jobid);
...
#          if(!globuscopy($jobid, $endpoint, $dst)) {
#            wlog DEBUG, "$jobid : Error in staging in $path -> $dst with Globus\n";
#            write $errpipe, "$jobid : Error in staging in $path -> $dst with Globus";
#          }
#        },
#        sub {
#          my ($err, $msg) = @_;
...
sub putFileCBDataIn {
	...
	wlog DEBUG, "$tag putFileCBDataIn\n";

	my $jobid = $$state{"jobid"};

	if (($flags & ERROR_FLAG) || $timeout) {
		if ($JOBDATA{$jobid}) {
			wlog DEBUG, "$jobid tag: $tag Stage out failed ($$replyref)\n";
			if ($timeout) {
				queueJobStatusCmd($jobid, FAILED, ERROR_STAGEOUT_TIMEOUT, "Stage out timed-out");
			}
			else {
				queueJobStatusCmd($jobid, FAILED, ERROR_STAGEOUT_SEND, "Stage out failed", $$replyref);
			}
			delete($JOBDATA{$jobid});
		}
		return;
	}
	elsif ($$replyref eq "STOP") {
		$SUSPENDED_TRANSFERS{"$tag"} = 1;
		wlog DEBUG, "$jobid tag: $tag Got stop request. Suspending transfer.\n";
	}
	elsif ($$replyref eq "CONTINUE") {
